...
if(isset($_GET["login"])){
    ...
    $query = "SELECT idadministrador, usuarioadministrador, CAST(AES_DECRYPT(contraseniaadministrador, '1234') AS CHAR) FROM administrador WHERE usuarioadministrador = '$usuario' AND contraseniaadministrador = AES_ENCRYPT('".$contrasenia."','1234')";
    ...
    if ($result->num_rows > 0) {
        ...
        $payload = array(
            ...
            'usuario' => $user['usuarioadministrador'],
            'role' => "Administrador"
        );
        $key = '1234Auy';
